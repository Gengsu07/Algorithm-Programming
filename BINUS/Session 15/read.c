    ...
    while (fgets(line, sizeof(line), mahasiswa) != NULL && structIndex < 5) // Read up to 5 students.
    {
        ...
        while (token != NULL)
        {
            ...
            case 0:
                ...
                kelas_a[structIndex].nim[sizeof(kelas_a[structIndex].nim) - 1] = '\0'; // Ensure null-termination
                break;
            case 1:
                strncpy(kelas_a[structIndex].nama, token, sizeof(kelas_a[structIndex].nama));
                kelas_a[structIndex].nama[sizeof(kelas_a[structIndex].nama) - 1] = '\0'; // Ensure null-termination
                break;
            case 2:
